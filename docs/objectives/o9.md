...
When you land on the correct account, the prompt rolls forward as well prompting us to see what containers exist and providing another hint to use `az storage container list` to list the container and its public access levels.
...
!!! success "Secrets"
    ...
    ```json title="Exposed Secrets" linenum="1" hl_lines="27,28"
