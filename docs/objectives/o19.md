...
After running each of the queries, there was a massive amount of information. I used the 'copy' function to copy and paste the results in to a file for easier searching. I spent quite a bit of time just scrolling through the information. The `gnomes` collection wasn't to bad. There were 17 gnome profiles with bits of information. The main thing I noticed at this time about the `gnomes` collection was that of the 17 profiles, each with a driver's license and profile image, only one driver's license was not accesible. There was the critical email information provided though for each user.
