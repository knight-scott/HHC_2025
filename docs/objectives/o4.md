...
Once again starting with a terminal, we are presented with the challenge:
...
We are presented with two tasks and a hint, with the first command to test. Hopping right in with the suggested command `ss -tlnp` returns
...
Ok, well that looks like the first task has been completed. We can see port 12321 is listening. Now we just need to connect to it to verify the `santa-tracking` service is running on the new port. We have just the terminal to use though so we'll need a terminal option to use, which the hint also points us to.<br/>

!!! success "Answer"
    ```bash title="CUrl Success"
